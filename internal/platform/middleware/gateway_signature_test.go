...
func TestVerifyGatewaySignature(t *testing.T) {
	...
	t.Run("valid GET passes", func(t *testing.T) {
		req := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodGet, "/v1/me/notifications", nil)
	...
	t.Run("valid POST with body passes", func(t *testing.T) {
		body := []byte(`{"ids":["n1","n2"]}`)
		req := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodPost, "/v1/me/notifications/read-all", body)
	...
	t.Run("cross-endpoint replay: same body different path rejected", func(t *testing.T) {
		body := []byte(`{"ids":["n1"]}`)
		reqA := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodPost, "/v1/me/notifications/read-all", body)
		sig := reqA.Header.Get(headerGatewaySignature)

		reqB := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodPost, "/v1/me/notifications/other", body)
	...
	t.Run("method-swap replay rejected", func(t *testing.T) {
		reqGet := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodGet, "/v1/me/notifications", nil)
		sig := reqGet.Header.Get(headerGatewaySignature)

		reqPost := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodPost, "/v1/me/notifications", nil)
	...
	t.Run("body tamper rejected", func(t *testing.T) {
		body := []byte(`{"ids":["n1"]}`)
		req := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodPost, "/v1/me/notifications/read-all", body)
		req.Body = io.NopCloser(bytes.NewReader([]byte(`{"ids":["evil"]}`)))
	...
	t.Run("stale timestamp rejected 401", func(t *testing.T) {
		...
		req := newSignedRequest(t, testSecret, fixAccountType, stale, http.MethodGet, "/v1/me/notifications", nil)
	...
	t.Run("future timestamp rejected 401", func(t *testing.T) {
		...
		req := newSignedRequest(t, testSecret, fixAccountType, future, http.MethodGet, "/v1/me/notifications", nil)
	...
	t.Run("empty account type stable positions passes", func(t *testing.T) {
		req := newSignedRequest(t, testSecret, "", now, http.MethodGet, "/v1/me/notifications", nil)
	...
	t.Run("dev empty secret skips verification", func(t *testing.T) {
		req := httptest.NewRequestWithContext(t.Context(), http.MethodGet, "/v1/me/notifications", http.NoBody)
		req.Header.Set(headerUserID, fixUserID)
	...
	t.Run("wrong secret rejected 401", func(t *testing.T) {
		req := newSignedRequest(t, "ffffffffffffffffffffffffffffffff", fixAccountType, now, http.MethodGet, "/v1/me/notifications", nil)
...
func TestVerifyGatewaySignature_NonceReplay(t *testing.T) {
	...
	t.Run("nil redis skips nonce check", func(t *testing.T) {
		...
		rec1 := runWithMiddleware(testSecret, nil,
			newSignedRequest(t, testSecret, fixAccountType, now, http.MethodGet, "/v1/me/notifications", nil))
	...
	t.Run("redis error is fail-closed", func(t *testing.T) {
		...
		ok := storeNonce(context.Background(), badRDB, "notif-nonce-bad")
	...
	t.Run("replay same requestId with redis is rejected", func(t *testing.T) {
		...
		now := time.Now().Unix()
		req1 := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodGet, "/v1/me/notifications", nil)
		req2 := newSignedRequest(t, testSecret, fixAccountType, now, http.MethodGet, "/v1/me/notifications", nil)

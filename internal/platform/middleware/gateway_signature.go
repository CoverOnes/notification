...
//
// This is defense-in-depth: the gateway signature ensures only gateway-forwarded
// requests reach protected routes — a forged request that bypasses the gateway is
// rejected before any identity-header logic executes.
//
...
func VerifyGatewaySignature(secret string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if err != nil || !withinGatewaySkew(tsInt) {
			abortUnauthorized(c)
			return
		}
...
// withinGatewaySkew reports whether the gateway timestamp is within the allowed replay
// window of the current time.
func withinGatewaySkew(tsUnix int64) bool {
...
// abortUnauthorized aborts with a generic 401 that does not leak which check
